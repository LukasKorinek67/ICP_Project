...
bool loadOBJ(const std::filesystem::path& path, std::vector < glm::vec3 > & out_vertices, std::vector < glm::vec2 > & out_uvs, std::vector < glm::vec3 > & out_normals) {
	...
	*/
	...
	while (1) {
		...
		*/
		char lineHeader[MAX_LINE_SIZE];
		if (fscanf(file, "%255s", lineHeader) == EOF) {
			break;
		}
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			*/
			...
			int matches = fscanf(file, "%d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d\n", &vertexIndex[0], &uvIndex[0], &normalIndex[0], &vertexIndex[1], &uvIndex[1], &normalIndex[1], &vertexIndex[2], &uvIndex[2], &normalIndex[2], &vertexIndex[3], &uvIndex[3], &normalIndex[3], &vertexIndex[4], &uvIndex[4], &normalIndex[4]);
			if (matches == 15) {
