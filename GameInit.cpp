...
bool Game::init() {
	...
	glCullFace(GL_BACK);		// Určuje, které strany (faces) polygonů by měly být odstraněny (cullovány) - zadní strany (které nejsou viditelné) nebudou vykreslovány - zvýšení výkonu
	glEnable(GL_CULL_FACE);		// Povolení odstranění zadních stran (face culling). OpenGL nebude vykreslovat zadní strany polygonů - zvýšení výkonu
	glDepthFunc(GL_LEQUAL);			// Nastavení funkce pro testování hloubky - GL_LEQUAL znamená, že pixel projde testem hloubky, pokud jeho hloubka je menší nebo rovna hodnotě uložené v hloubkovém bufferu
	glEnable(GL_DEPTH_TEST);	// Povolení testování hloubky - OpenGL používá hloubkový buffer pro určení, které objekty jsou před nebo za jinými objekty
	glEnable(GL_LINE_SMOOTH);	// Povolení vyhlazování čar - budou vypadat méně zubatě
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);	// Nastavuje funkci pro míchání (blending) barev. Pokud má objekt alfa hodnotu 0.5, bude tento objekt poloprůhledný
...
void Game::init_assets() {
	...
	// délka 5,1f, šířka 1,95f, výška 1,2f
	...
	// délka 4,2f, šířka 1,65f, výška 1,05f
...
void Game::init_glfw(void) {
	...
	window = glfwCreateWindow(1280, 800, "ICP Project - Kořínek & Doanová", NULL, NULL);
